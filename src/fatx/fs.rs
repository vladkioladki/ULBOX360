...
use crate::fatx::fat::{ClusterId, Fat};
    ...
    pub fn open_device(config: &FatxFsConfig) -> Result<FatxFsHandle, Error> {
        ...
        if config.partition_offset_bytes + actual_partition_size > file_len {
            if file_len > config.partition_offset_bytes {
                actual_partition_size = file_len - config.partition_offset_bytes;
            } else {
                return Err(Error::InvalidPartitionOffset);
            }
        }
        ...
        let root_cluster: u32 = superblock.root_cluster.get();
        ...
        let num_clusters =
            ((actual_partition_size - fat.fat_size_bytes - FATX_FAT_OFFSET_BYTES)
        ...
        let fs = Arc::new_cyclic(move |weak_self| {
            Mutex::new(FatxFs {
                ...
                partition_size_bytes: actual_partition_size,
                num_clusters,
                num_bytes_per_cluster,
